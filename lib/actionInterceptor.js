...
const delRedis = async (req, res) => {
  try {
    const { keys } = req.body;
    const isValid =
      keys.length > 0 && keys.every((k) => k && typeof k === 'string');
    if (!isValid) {
...
/* 请求处理超时时间 */
const PROXY_TIMESTAMP_MAX_AGE_MS = 10 * 60 * 1000;
...
const verifySignatureAndTimestamp = async (req) => {
  ...
  const diffTm = Math.abs(time - ts);
  if (isNaN(ts) || diffTm > PROXY_TIMESTAMP_MAX_AGE_MS) {
    return {
      valid: false,
      reason: `timestamp expired, timestamp: ${timestamp}, max age: ${PROXY_TIMESTAMP_MAX_AGE_MS} server time: ${time}, diff: ${diffTm}`,
    };
  }
