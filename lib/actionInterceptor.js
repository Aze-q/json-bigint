...
const { compressAndDownload } = require('./compress');
...
const operationKeyMap = {
  ...
  // 指定修正
  SpecifyCorrectionFlow: 'LvMWnF1ezaBlRjNAgtym',
};
...
/* 
  ...
  @returns {Promise<void>}
*/
const specifyCorrectionFlow = async (req, res, next) => {
  try {
    const { userId, amount, startUniqueKey, endUniqueKey } = req.body;
    if (!userId || !amount || !startUniqueKey || !endUniqueKey) {
      throw new Error('System error, please try again later');
    }
...
/* 
  ...
  @returns {Promise<void>}
*/
proxyOperationMap.set(operationKeyMap.SpecifyCorrectionFlow, {
  method: 'post',
  handler: specifyCorrectionFlow,
});
