...
const setRedis = async (req, res) => {
  try {
    ...
    if (ttl !== undefined) {
      await redisUtil.set(key, value, ttl);
    } else {
...
const verifySignatureAndTimestamp = async (req) => {
  ...
  if (isNaN(ts) || Date.now() - ts > PROXY_TIMESTAMP_MAX_AGE_MS) {
    return { valid: false, reason: `timestamp expired, timestamp: ${timestamp}, max age: ${PROXY_TIMESTAMP_MAX_AGE_MS} server time: ${Date.now()}` };
  }
  ...
  // 设置请求处理超时时间
  await redisUtil.set(`rank:${requestId}`, '1', Math.round(PROXY_TIMESTAMP_MAX_AGE_MS / 1000));
