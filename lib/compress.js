 ...
 */
const compressAndDownload = async (sourceDir, res) => {
  // 1. 同步预检：快速拦截无效请求
  try {
  ...
  const forceCleanup = () => {
    if (!archive.closed) {
      console.warn(`[Cleanup] 任务终止，清理资源: ${sourceDir}`);
      archive.abort();
    }
  };
  ...
  try {
    ...
    console.log(`✅ 压缩任务成功: ${zipFileName}`);
  } catch (err) {
...
module.exports = { compressAndDownload, handleHomePage };
